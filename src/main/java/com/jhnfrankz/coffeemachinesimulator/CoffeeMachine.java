...
public class CoffeeMachine {
    ...
    public static void readAction(CoffeeMachine coffeeMachine) {
        System.out.println("Write action (buy, fill, take, remaining, exit):");
        String option = scanner.nextLine();

        switch (option) {
            case "buy":
                coffeeMachine.setStateMachine(StateMachine.BUY);
                buy();
                break;
            case "fill":
                coffeeMachine.setStateMachine(StateMachine.FILL);
                fill();
                break;
            case "take":
                coffeeMachine.setStateMachine(StateMachine.TAKE);
                take();
                break;
            case "remaining":
                coffeeMachine.setStateMachine(StateMachine.REMAINING);
                showStateMachine();
                break;
            case "exit":
                coffeeMachine.setStateMachine(StateMachine.EXIT);
            default:
                break;
        }
    }
